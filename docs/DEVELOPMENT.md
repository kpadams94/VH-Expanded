...
3. Add a newest-first version to `CHANGELOG.md`, with **Added**, **New Mods**, **Balance**, and **Bug Fixes** sections. Use brief, player-facing entries, normally one sentence per feature or mod. Include longer explanations only when needed to understand the change; icons and other constituent parts belong under their feature rather than separate entries. Leave unused sections empty. Initial-release development corrections are not fixes to an earlier public release.
