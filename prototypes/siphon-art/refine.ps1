...
$edits=@(@{x=7;y=7;rgba='FFF2BCFF'},@{x=9;y=5;rgba='FFAD88FF'},@{x=11;y=7;rgba='FFAD88FF'},@{x=2;y=14;rgba='00000000'},@{x=4;y=14;rgba='00000000'},@{x=3;y=15;rgba='00000000'})
...
$im.Save((Join-Path $root 'native/siphon-v1.png'),[Drawing.Imaging.ImageFormat]::Png)
$rows=@();for($y=0;$y -lt 16;$y++){$row=@();for($x=0;$x -lt 16;$x++){$p=$im.GetPixel($x,$y);$row+=('{0:X2}{1:X2}{2:X2}{3:X2}' -f $p.R,$p.G,$p.B,$p.A)};$rows+=,@($row)}
@{width=16;height=16;format='RGBA hex rows';pixels=$rows;editorEdits=$edits;provenance='Native-grid drawing and replay of six recorded Piskel edits.'}|ConvertTo-Json -Depth 8|Set-Content -Encoding utf8 (Join-Path $root 'native/siphon-v1.source.json')
...
$g.DrawString('SIPHON / 16 x 16 draft',$font,[Drawing.Brushes]::White,24,16);$g.DrawString('Actual PNG at 1x, 3x and 16x (nearest-neighbor)',$small,[Drawing.Brushes]::White,24,47)
...
foreach($entry in @(@('Lunge','../../docs/art/lunge/lunge.png',140),@('Impact','../../docs/art/impact/impact.png',245))){$ref=[Drawing.Bitmap]::new((Join-Path $root $entry[1]));$g.DrawString($entry[0],$small,[Drawing.Brushes]::White,610,($entry[2]-25));$g.DrawImage($ref,[Drawing.Rectangle]::new(610,$entry[2],48,48));$ref.Dispose()}
$g.Dispose();$b.Save((Join-Path $root 'siphon-v1-review.png'));$b.Dispose();$im.Dispose()
